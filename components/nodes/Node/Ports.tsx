...
export interface INodePortsProps {
	node: _Node;
	omit?: Array<_Port<any>>;
	disabled?: Array<_Port<any>>;
}

export const InputPorts = observer(
	({ node, omit, disabled }: INodePortsProps) => {
			...
			<div className="absolute flex flex-col justify-evenly h-[calc(100%-32px)] top-8 -left-3">
				{inputPorts.map((port, index) => (
					<Port
						...
						disabled={disabled ? disabled.includes(port) : false}
						isPositioned={!!port.data.isPositioned}
					/>
				))}
			</div>
		);
...
export const OutputPorts = observer(
	({ node, omit, disabled }: INodePortsProps) => {
		const omitIds = omit?.map((port) => port.id);

		const outputPorts = Object.values(node.outputPorts).filter(
			(port) => !omitIds || !omitIds.includes(port.id)
		);
...
export const Port = observer(
	({ port, index, total, disabled = false, isPositioned = false }: INodePortProps) => {
		...
		useEffect(() => {
			...
			const x =
				port.type === PortType.OUTPUT ? port.node.data.size.width : 0;
		...
		return (
			<button
				className={`flex justify-center items-center w-6 h-6 text-[10px] font-medium border-2 border-zinc-200 dark:border-zinc-700 text-zinc-400 dark:text-zinc-500 rounded-full bg-offwhite dark:bg-zinc-900 ${
					editor.connectingPort === port
					...
					disabled
						...
						: "hover:border-zinc-400 dark:hover:border-zinc-400 cursor-pointer"
				} ${isPositioned ? "absolute" : ""}`}
				style={
					port.type === PortType.INPUT
						? {
								left: port.data.position
									? port.data.position.x
									: 0,
			...
			>
				...
				{port.isConnected && port.type === PortType.INPUT && (
					<button
						className={
							"absolute flex items-center justify-center w-6 h-6 transition-opacity bg-red-400 border-2 border-red-500 rounded-full opacity-0 dark:bg-red-500 dark:border-red-400 hover:opacity-100 text-offwhite"
						}
					>
						<Cross2Icon />
					</button>
